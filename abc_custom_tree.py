# MutSimABC Application Novel Tree
...
tree_topologies_dict = {
    ## MODIFY SECTION 1##
    # Add your custom tree here
    "my_custom_tree": {
        "numBranch": 6,           # Number of terminal branches you sampled
        "age": 150,               # Root-to-tip distance in years
        "s10": 10,                # Trunk before first split (typically 10)
        
        # Right side branches
        "b11": 40,                # Internal branch ages (years)
        "bb11": 25,               # Terminal branch ages (years)
        "b12": 40,
        "bb12": 25,
        "b13": 25,                # Last internal (treated as terminal)
...
# EPSILON = 20 (CHANGE THIS- SENSITIVITY ANALYSIS REQUIRED)
...
## MODIFY SECTION 4 ##
def sample_prior():
    StD = np.random.uniform(0, 5) # KEEP OR DELETE STD=0 HERE-> MODEL JUMPING SEE RESULTS/METHODS
    biasVar = np.random.uniform(0.5, 10)
    input_mut = np.random.uniform(5.0E-11, 1.12E-09)#UPDATE INPUT_MUT TO REFLECT RANGE OF FEASIBLE MUT RATE - AGAIN SENSITIVITY ANALYSIS RECOMMENDED
...
def run_simulation(StD, biasVar, input_mut):
    ...
    NumTime = 1
    ## MODIFY SECTION 2 ##
    #Change 'test' to your tree name
    tree_list, tree_dict, numBranch, age = create_tree_list_and_dict(tree_topologies_dict['my_custom_tree']) # UR TREE HERE <-
...
def calculate_distance(unique_mutations):
    real_distribution = [37.89349405, 37.99117253, 35.91959651, 42.62965378, 
                         38.59110184, 30.84725743, 32.74895451, 43.37876936] # IMPORTANT- ADD TRUE VARIANT NUMBERS PER TERMINAL BRANCH HERE
...
def save_accepted_samples(accepted_samples, job_id, batch_id, epsilon):
    ...
    output_dir = "results_my_tree"# CHANGE DIRECTORY HERE (Optional)
