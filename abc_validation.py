...
def parse_unique_mutations(unique_mutations_str):
    try:
        return ast.literal_eval(unique_mutations_str)
    except (ValueError, SyntaxError) as e:
        print(f"Error parsing unique mutations: {e}")
        return []
...
# Run simulation (per sample, using unique mutations as real data)
def run_simulation(StD, biasVar, input_mut, tree_topology):
    """
    Run the simulation using the parameters and the appropriate tree topology
    from the 'tree_topology' column in the sample.
    """
    GenSize = 500000000
    NumStem = 5
    nDiv = 1
    ...
    # Dynamically select the correct tree_dict based on the tree_topology
    tree_list, tree_dict, numBranch, age = create_tree_list_and_dict(tree_topologies_dict[tree_topology])

    # Simulate somatic mutations with the appropriate tree_dict
    resultList_2 = simulate_somatic_mutations(
        tree_dict, NumStem=int(NumStem), NumTime=int(NumTime),
        mu_0=input_mut, GenSize=int(GenSize), 
        StD=StD, NumDiv=NumDiv, nDiv=nDiv, biasVar=biasVar
    )

    # Extract mutation-related metrics
    ...
    variants, regression_equation, max_root_to_tip_distance, output_mut_rate = calc_variants(
        genetic_matrix, physical_matrix, tree_dict, GenSize
    )
...
# Save ACCEPTED samples only!
def save_accepted_samples(accepted_samples, sample_id, epsilon):
    timestamp = time.strftime("%Y%m%d-%H%M%S")
    output_dir = "result_valid"
    os.makedirs(output_dir, exist_ok=True)

    # Save with filename indicating sample ID
    output_file = os.path.join(output_dir, f"accepted_samples_sample{sample_id}_epsilon{epsilon}_{timestamp}.csv")
    pd.DataFrame(accepted_samples, columns=["Sample_ID", "StD", "biasVar", "input_mut", "output_mut_rate"]).to_csv(output_file, index=False)
    print(f"Accepted samples for sample {sample_id} saved to {output_file}")

# ABC-Rejection for a single validation sample
def abc_rejection_for_sample(real_distribution, sample_id, epsilon, tree_topology):
    """
    Run ABC rejection sampling for a given sample using the specified tree topology.
    Ensure that exactly 100 accepted samples are generated.
    """
    accepted_samples = []

    while len(accepted_samples) < 100:  # Continue until we collect 100 accepted samples
        # Sample from the prior
        StD, biasVar, input_mut = sample_prior()

        # Run simulation with the correct tree topology for the current sample
        unique_mutations, output_mut_rate = run_simulation(StD, biasVar, input_mut, tree_topology)

        # Calculate distance and accept/reject
        dist = calculate_distance(unique_mutations, real_distribution)
        if dist < epsilon:
            accepted_samples.append([sample_id, StD, biasVar, input_mut, output_mut_rate])

    # Save accepted samples for this sample ID
    save_accepted_samples(accepted_samples, sample_id, epsilon)

# Run ABC validation for the specific sample based on SLURM_ARRAY_TASK_ID
def run_abc_validation(task_id):
    """
    Run ABC rejection sampling for a single sample based on the SLURM_ARRAY_TASK_ID.
    """
    epsilon = 20  # Epsilon threshold for acceptance

    # Get the sample based on the task ID
    row = df_validation.iloc[task_id]
    sample_id = task_id  # Task ID matches the sample ID
    real_distribution = parse_unique_mutations(row['Unique_Mutations'])  # Parse 'real' distribution
    tree_topology = row['tree_topology']  # Get the correct tree topology for this sample
    print(f"Running ABC rejection for sample {sample_id} with tree topology {tree_topology}...")

    # Run ABC rejection for this sample
    abc_rejection_for_sample(real_distribution, sample_id, epsilon, tree_topology)

# Execute the ABC validation process for the current task ID
if __name__ == "__main__":
    # Get the SLURM_ARRAY_TASK_ID passed from the SLURM script
    task_id = int(sys.argv[1])
    run_abc_validation(task_id)
